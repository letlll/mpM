	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item v-else class="input" label="分类" prop="fenlei" >
		...
		data() {
			...
			return {
				...
				rules: {
					...
					shouzhuren: [
						{ required: true, message: '受助人不能为空', trigger: 'blur' },
					],
					lianxifangshi: [
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `shouzhuzhe/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.shouzhuzheCrossAddOrUpdateFlag = false;
											this.parent.search();
