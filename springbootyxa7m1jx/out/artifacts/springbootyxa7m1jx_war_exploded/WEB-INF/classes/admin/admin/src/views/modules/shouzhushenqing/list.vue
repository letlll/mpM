		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"10px","background":"#fff","display":"block"}' >
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"inline","lineHeight":"32px","fontSize":"inherit","fontWeight":"500","height":"32px"}' class="item-label">申请标题</label>
						<el-input v-model="searchForm.shenqingbiaoti" placeholder="申请标题" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}' class="select">
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"inline","lineHeight":"32px","fontSize":"inherit","fontWeight":"500","height":"32px"}' class="item-label">是否通过</label>
				...
				<el-row class="actions" :style='{"padding":"20px 30px","margin":"10px 0 0","borderRadius":"4px 4px 0 0","flexWrap":"wrap","background":"#fff","display":"flex"}'>
					<el-button class="add" v-if="isAuth('shouzhushenqing','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia14" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"inline","height":"34px"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('shouzhushenqing','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu6" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"inline","height":"34px"}'></span>
						删除
					</el-button>


					<el-button class="btn18" v-if="isAuth('shouzhushenqing','审核')" :disabled="dataListSelections.length?false:true" type="success" @click="shBatchDialog()">
			...
			<div :style='{"width":"100%","padding":"0 30px","backgroundColor":"#fff","fontSize":"14px","color":"#000"}'>
				<el-table class="tables"
					...
					:border='true'
					v-if="isAuth('shouzhushenqing','查看')"
					:data="dataList"
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true'  
						...
						label="申请标题">
						<template slot-scope="scope">
							{{scope.row.shenqingbiaoti}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' prop="shenqingwenjian" label="申请文件">
						<template slot-scope="scope">
							<el-button v-if="scope.row.shenqingwenjian" type="text" size="small" @click="download(scope.row.shenqingwenjian)">下载</el-button>
							<span v-else >无</span>
						</template>
					</el-table-column>
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('shouzhushenqing','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="edit" v-if=" isAuth('shouzhushenqing','修改')  && scope.row.sfsh=='待审核' " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								修改
							</el-button>
							...
							<el-button class="del" v-if="isAuth('shouzhushenqing','删除') " type="primary" @click="deleteHandler(scope.row.id )">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								删除
							</el-button>
						</template>
	...
	export default {
		...
		methods: {
			...
			getDataList() {
				...
				params['order'] = 'desc';
 				if(this.searchForm.shenqingbiaoti!='' && this.searchForm.shenqingbiaoti!=undefined){
					params['shenqingbiaoti'] = '%' + this.searchForm.shenqingbiaoti + '%'
				}
				...
				this.$refs["shBatchForm"].validate(valid => {
					if(valid){
						this.$confirm(`是否${this.batchIds.length>1?'一键审核':'审核'}选中数据?`, "提示", {
							...
							this.$http({
								url: "shouzhushenqing/shBatch?sfsh="+this.shBatchForm.sfsh+"&shhf="+this.shBatchForm.shhf,
								method: "post",
								data: this.batchIds
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
