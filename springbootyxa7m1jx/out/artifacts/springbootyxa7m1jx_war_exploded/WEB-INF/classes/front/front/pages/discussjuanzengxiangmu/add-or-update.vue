	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discussjuanzengxiangmu`, this.ruleForm.id);
		...
		methods: {
			...
			async onSubmitTap() {
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discussjuanzengxiangmu`, params);
				...
				//跨表计算
						let oet = {}
						if(this.ruleForm.id){
							await this.$api.update(`discussjuanzengxiangmu`, this.ruleForm);
						}else{
							oet = await this.$api.add(`discussjuanzengxiangmu`, this.ruleForm);
						}
						uni.setStorageSync('discussjuanzengxiangmuCleanType',true);
