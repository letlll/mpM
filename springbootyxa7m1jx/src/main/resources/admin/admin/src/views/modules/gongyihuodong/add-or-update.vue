		...
		data() {
			...
			return {
				...
				rules: {
					...
					jubanfang: [
						{ required: true, message: '举办方不能为空', trigger: 'blur' },
					],
					clicktime: [
					],
					reversetime: [
			...
			info(id) {
				this.$http({
					...
					if (data && data.code === 0) {
						...
						this.ruleForm.huodongneirong = this.ruleForm.huodongneirong.replace(reg,'../../../springbootyxa7m1jx/upload');
					} else {
						this.$message.error(data.msg);
					}
				});
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `gongyihuodong/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.gongyihuodongCrossAddOrUpdateFlag = false;
											this.parent.search();
