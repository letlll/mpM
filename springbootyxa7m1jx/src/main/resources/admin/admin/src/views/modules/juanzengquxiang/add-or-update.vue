<template>
	<div class="addEdit-block">
		...
		>
			...
			<el-form-item v-if="type!='info'"  label="去向说明" prop="quxiangshuoming" >
				<editor 
					style="min-width: 200px; max-width: 600px;"
					v-model="ruleForm.quxiangshuoming" 
					class="editor"
					myQuillEditor="quxiangshuoming"
					action="file/upload">
				</editor>
			</el-form-item>
			<el-form-item v-else-if="ruleForm.quxiangshuoming" label="去向说明" prop="quxiangshuoming" >
				<span class="text ql-snow ql-editor" v-html="ruleForm.quxiangshuoming"></span>
			</el-form-item>
			...
			init(id,type) {
				...
				if(this.type=='info'||this.type=='else'||this.type=='msg'){
					...
					for (var o in obj){
						...
						if(o=='quxiangshuoming'){
							this.ruleForm.quxiangshuoming = obj[o];
							this.ro.quxiangshuoming = true;
							continue;
						}
						if(o=='gengxinshijian'){
			...
			info(id) {
				this.$http({
					...
					if (data && data.code === 0) {
						...
						this.ruleForm.quxiangshuoming = this.ruleForm.quxiangshuoming.replace(reg,'../../../springbootyxa7m1jx/upload');
					} else {
						this.$message.error(data.msg);
					}
				});
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `juanzengquxiang/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.juanzengquxiangCrossAddOrUpdateFlag = false;
											this.parent.search();
