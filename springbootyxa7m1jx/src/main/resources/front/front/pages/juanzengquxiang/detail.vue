	...
	<view class="content">
		<view class="container" :style='{"minHeight":"100%","width":"100%","padding":"0","position":"relative","background":"#E9E9E9","height":"auto"}'>
			...
			<view :style='{"padding":"20rpx","margin":"-80rpx 0 0","background":"none","width":"100%","position":"relative","height":"auto","zIndex":"9"}' class="detail-content">
				...
				<view class="detail-list-item" :style='{"padding":"0","margin":"0","borderColor":"#ccc","borderWidth":"0 0 2rpx 0","background":"#FFFFFF","display":"flex","width":"100%","borderStyle":"solid","height":"auto"}'>
					<view class="lable" :style='{"width":"160rpx","padding":"0 20rpx 0 0","lineHeight":"80rpx","fontSize":"28rpx","color":"#929292","textAlign":"right"}'>更新时间：</view>
				...
				<view class="detail-list-item rich" :style='{"margin":"0","borderColor":"#ccc","borderWidth":"0 0 2rpx 0","background":"#fff","display":"flex","width":"100%","borderStyle":"solid","height":"auto"}'>
					<view class="lable" :style='{"width":"160rpx","padding":"0 20rpx 0 0","lineHeight":"80rpx","fontSize":"28rpx","color":"#929292","textAlign":"right"}'>去向说明：</view>
					<view class="rich-text" :style='{"padding":"24rpx 24rpx 24rpx 0","margin":"0"}'>
						<rich-text :nodes="detail.quxiangshuoming"></rich-text>
					</view>
				</view>
				...
				<view class="bottom-content bg-white tabbar border shop" :style='{"padding":"10rpx 24rpx","flexWrap":"wrap","background":"#FFFFFF","display":"flex","gap":"20rpx","width":"100%","height":"auto"}'>
					...
					<button :style='{"border":"0","padding":"0 20rpx","margin":"0","color":"rgb(255, 255, 255)","borderRadius":"8rpx","background":"linear-gradient(45deg, #3d4e81, #5753c9, #6e7ff3)","width":"auto","fontSize":"28rpx","height":"80rpx"}' v-if="userid&&isAuth('juanzengquxiang','私聊')" @tap="chatClick">联系TA</button>
					<button :style='{"border":"0","padding":"0 20rpx","margin":"0","color":"rgb(255, 255, 255)","borderRadius":"8rpx","background":"linear-gradient(45deg, #3d4e81, #5753c9, #6e7ff3)","width":"auto","fontSize":"28rpx","height":"80rpx"}' v-if="!userid&&isAuthFront('juanzengquxiang','私聊')" @tap="chatClick">联系TA</button>
	...
	export default {
		...
		async onShow(options) {
			...
			if(crossCleanType) {
				...
				res = await this.$api.info('juanzengquxiang', this.id);
				let reg=new RegExp('http://localhost:8080/springbootyxa7m1jx/upload','g')//g代表全部
				if(res.data.quxiangshuoming){
					res.data.quxiangshuoming = res.data.quxiangshuoming.replace(reg, this.$base.url + 'upload');
				}
		...
		methods: {
			...
			onPayTap(){
				...
				uni.setStorageSync('paytable','juanzengquxiang');
			...
			async onAcrossTap(tableName,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
				...
				uni.setStorageSync('crossTable','juanzengquxiang');
			...
			async init(type=1){
				...
				let res = await this.$api.info('juanzengquxiang', this.id);
				let reg=new RegExp('http://localhost:8080/springbootyxa7m1jx/upload','g')//g代表全部
				res.data.quxiangshuoming = res.data.quxiangshuoming.replace(reg, this.$base.url + 'upload');
				this.detail = res.data;
				...
				//修改富文本的图片样式
				this.detail.quxiangshuoming = this.detail.quxiangshuoming.replace(/img src/gi,"img style=\"width:100%;\" src");



				if(type==2){
					this.detail.discussnum++
					await this.$api.update('juanzengquxiang',this.detail)
				}
			},
